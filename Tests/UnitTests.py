...
Jobs_UserVolumes = [
    {
        "name": Jobs_UserVolumeName,
        "rootVolumeName": Jobs_RootVolumeName,
        "owner": Authentication_loginName,
...
class TestAuthentication(unittest.TestCase):
    ...
    def test_Authentication_allMethods(self):
        ...
        token1 = Authentication.login(
            Authentication_loginName, Authentication_loginPassword
        )
...
class TestCasJobs(unittest.TestCase):
    ...
    def test_CasJobs_executeQuery(self):
        df = CasJobs.executeQuery(
            sql=CasJobs_TestQuery, context=CasJobs_TestDatabase, format="pandas"
        )
        self.assertEqual(CasJobs_TestTableCSV, df.to_csv(index=False))

    def test_CasJobs_submitJob(self):
        jobId = CasJobs.submitJob(
            sql=CasJobs_TestQuery + " into MyDB." + CasJobs_TestTableName1,
            context=CasJobs_TestDatabase,
        )
        jobDescription = CasJobs.waitForJob(jobId=jobId, verbose=True)
        df = CasJobs.executeQuery(
            sql="DROP TABLE " + CasJobs_TestTableName1, context="MyDB", format="csv"
        )
    ...
    def test_CasJobs_getPandasDataFrameFromQuery(self):
        ...
        df = CasJobs.getPandasDataFrameFromQuery(
            queryString=CasJobs_TestQuery, context=CasJobs_TestDatabase
        )
    ...
    def test_CasJobs_getNumpyArrayFromQuery(self):
        ...
        newArray = pandas.read_csv(
            StringIO(CasJobs_TestTableCSV), index_col=None
        ).as_matrix()
    ...
    def test_CasJobs_uploadPandasDataFrameToTable_uploadCSVDataToTable(self):
        try:
            ...
            table = CasJobs.executeQuery(
                sql="select * from " + CasJobs_TestTableName2,
                context="MyDB",
                format="pandas",
            )
            result2 = CasJobs.executeQuery(
                sql="DROP TABLE " + CasJobs_TestTableName2, context="MyDB", format="csv"
            )
            self.assertEqual(result, True)
            ...
            try:
                csv = CasJobs.executeQuery(
                    sql="DROP TABLE " + CasJobs_TestTableName2,
                    context="MyDB",
                    format="csv",
                )
            except:
...
class TestSkyServer(unittest.TestCase):
    ...
    def test_SkyServer_sqlSearch(self):
        # sql search
        df = SkyServer.sqlSearch(
            sql=SkyServer_TestQuery, dataRelease=SkyServer_DataRelease
        )
    ...
    def test_SkyServer_radialSearch(self):
        # radial search
        df = SkyServer.radialSearch(
            ra=258.25, dec=64.05, radius=0.1, dataRelease=SkyServer_DataRelease
        )
        self.maxDiff = None
        self.assertEqual(
            SkyServer_RadialSearchResultCSV, df.to_csv(index=False, float_format="%.6f")
        )
    ...
    def test_SkyServer_objectSearch(self):
        # object search
        object = SkyServer.objectSearch(
            ra=258.25, dec=64.05, dataRelease=SkyServer_DataRelease
        )
        self.maxDiff = None
...
class TestSciDrive(unittest.TestCase):
    ...
    def test_SciDrive_upload_download_delete(self):
        try:
            ...
            responseUpload = SciDrive.upload(
                path=SciDrive_Directory + "/" + SciDrive_FileName,
                localFilePath=SciDrive_FileName,
            )

            stringio = SciDrive.download(
                path=SciDrive_Directory + "/" + SciDrive_FileName, format="StringIO"
            )
            ...
            fileContent = SciDrive.download(
                path=SciDrive_Directory + "/" + SciDrive_FileName, format="text"
            )
            responseDelete = SciDrive.delete(SciDrive_Directory)
            self.assertEqual(
                responseUpload["path"], SciDrive_Directory + "/" + SciDrive_FileName
...
class TestSkyQuery(unittest.TestCase):
    ...
    def test_SkyQuery_uploadTable_getTable_getTableInfo_listTableColumns_dropTable(
        ...
        try:
            result = SkyQuery.dropTable(
                tableName=SkyQuery_TestTableName, datasetName="MyDB"
            )
        except:
            pass

        result = SkyQuery.uploadTable(
        ...
        table = SkyQuery.getTable(
            tableName=SkyQuery_TestTableName, datasetName="MyDB", top=10
        )
        self.assertEqual(SkyQuery_TestTableCSVdownloaded, table.to_csv(index=False))

        info = SkyQuery.getTableInfo(
            tableName="webuser." + SkyQuery_TestTableName, datasetName="MyDB"
        )

        columns = SkyQuery.listTableColumns(
            tableName="webuser." + SkyQuery_TestTableName, datasetName="MyDB"
        )
...
class TestFileService(unittest.TestCase):
    ...
    def test_Files_createUserVolume_deleteUserVolume(self):
        ...
        Files.createUserVolume(
            fileService,
            "/".join(
                [Files_RootVolumeName1, Authentication_loginName, Files_UserVolumeName1]
            ),
            quiet=False,
        )
        Files.deleteUserVolume(
            fileService,
            "/".join(
                [Files_RootVolumeName1, Authentication_loginName, Files_UserVolumeName1]
            ),
    ...
    def test_Files_createDir_upload_dirList_download_download_shareUserVolume(self):
        ...
        try:
            ...
            Files.deleteUserVolume(
                fileService, Files_RootVolumeName1, Files_UserVolumeName1, quiet=True
            )
            Files.deleteUserVolume(
                fileService, Files_RootVolumeName1, Files_UserVolumeName2, quiet=True
            )
        except:
            pass
        ...
        try:
            ...
            self.assertTrue(
                dirList.get("root").get("files")[0].get("name") == Files_LocalFileName
            )
...
class TestJobs(unittest.TestCase):
    ...
    def test_Jobs_submitNotebookJob_cancel_waitForJob_getJobStatus_getJobDescription_submitShellCommandJob(
        ...
        try:
            Files.deleteUserVolume(
                fileService, Jobs_RootVolumeName, Jobs_UserVolumeName
            )
        except:
            pass
        ...
        jobId = Jobs.submitShellCommandJob(
            Jobs_ShellCommand,
            dockerComputeDomain,
            Jobs_DockerImageName,
            Jobs_UserVolumes,
    ...
    def test_Jobs_submitRDBJob(self):
        ...
        jobId = Jobs.submitRDBQueryJob(
            Jobs_SqlQuery,
            rdbComputeDomain,
            Jobs_DatabaseContextName,
            Jobs_QueryResultsFile,
            Jobs_Alias,
        )
        ...
        jobDirectory = job.get("resultsFolderURI")
        relativePath = (
            jobDirectory.split("scratch/")[1] + Jobs_QueryResultsFile + ".csv"
        )
        string = Files.download(
            fileService,
            "scratch",
...
if __name__ == "__main__":
    ...
    suite = testLoader.loadTestsFromTestCase(TestAuthentication)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestLoginPortal)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestCasJobs)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestSkyServer)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestSciDrive)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestSkyQuery)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestFileService)
    unittest.TextTestRunner(verbosity=2).run(suite)
    suite = testLoader.loadTestsFromTestCase(TestJobs)
    unittest.TextTestRunner(verbosity=2).run(suite)
